...
void on_timer(htimer_t *timer) {
    ...
    printf("on_timer: event_id=%llu\tpriority=%d\tuserdata=%ld\ttime=%llus\thrtime=%lluus\n", LLU(hevent_id(timer)), hevent_priority(timer),
           (long)(intptr_t)(hevent_userdata(timer)), LLU(hloop_now(loop)), LLU(hloop_now_hrtime(loop)));
}
...
int test_hloop() {
    ...
    for (int i = HEVENT_LOWEST_PRIORITY; i <= HEVENT_HIGHEST_PRIORITY; ++i) {
        hidle_t *idle = hidle_add(loop, on_idle, 1);
        hevent_set_priority(idle, i);
    }

    // test timer timeout
    // for (int i = 1; i <= 10; ++i) {
    htimer_t *timer = htimer_add(loop, on_timer, 10, 3);
    ...
    int minute = 1;
    htimer_add_period(loop, cron_hourly, minute, -1, -1, -1, -1, INFINITE);
